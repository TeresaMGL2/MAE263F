%(Junlin Chen, 505947745, load_train_and_test_data.m)
...
%      Y_test: a K x N_t matrix representing the labels of the testing images, where each column corresponds to a one-hot encoding of a single image.
    ...
    pixel = reshape(pixel, [size(pixel, 1)*size(pixel, 2), size(pixel, 3)]);
    X_train = pixel/range(pixel,'all'); %normalize
    % Convert y to one-hot encoding
    Y_train = zeros(10, size(label,2)); %onehot encoding
    ...
    pixel = reshape(pixel, [size(pixel, 1)*size(pixel, 2), size(pixel, 3)]);
    X_test = pixel/range(pixel,'all'); %normalize
    Y_test = zeros(10, size(label,2)); %onehot encoding
    for i = 1:size(label,2)
