...
void world::updateTimeStep() {
    if (time_step == 150) { //stop applying force after x time step
        auto it = forces->forces.begin() + 1;
        forces->forces.erase(it);
        printf("force lifted!!\n");
    }
...
void world::printSimData()
{
    ...
    if (cf && ff) {
        ...
        int nvn = soft_robots->limbs[7]->nv; //get number of vertices in a limb
        if ((soft_robots->limbs[7]->u[4*round(nvn/2)+2])>max_v and ff->min_dist > 0.02){
            max_v = (soft_robots->limbs[7]->u[4*round(nvn/2)+2]);
        }
        if (0.3-soft_robots->limbs[7]->x[4*round(nvn/2)+2]>max_c){
            max_c = 0.3-soft_robots->limbs[7]->x[4*round(nvn/2)+2];
        }
        printf("time: %.4f | iters: %i | floor_con: %i | f_min_dist: %.6f\n | compressed_dist: %.6f\n | velocity: %.6f\n",
               curr_time, stepper->iter, ff->num_contacts, ff->min_dist, 0.3-soft_robots->limbs[7]->x[4*round(nvn/2)+2], soft_robots->limbs[7]->u[4*round(nvn/2)+2]);
    }
    else {
